...
import glob
import math  # Aggiunto per pigreco

def main():
    # Intestazione e sottotitolo - AGGIORNATO A 1.1.3
    header_title = "Triboanalisi 1.1.3"
    header_subtitle = "TRIBOANALISI 1.1.3 © 2026 by Francesco Cammelli is licensed under GNU GPL v3 (General Public License)"
    ...
    while True:
        ...
        print("- Calcolare i dati sperimentali come mm/s e CoF")
        ...
        # ============================================================
        # NUOVA FUNZIONE: 11. Calcolo del coefficiente di attrito
        # ============================================================
        current_step = "11. Calcolo del coefficiente di attrito"
        print(f"\n=== {current_step} ===")
        step_stack.append(current_step)
        
        # Chiedi all'utente se vuole aggiungere le colonne mm/s e CoF
        while True:
            add_cof = input("\nVuoi aggiungere le colonne 'mm/s' e 'CoF' (coefficiente di attrito) al file di output? (s/n o 'exit'/'back'): ").strip().lower()
            if add_cof == 'exit':
                print("\nGrazie per aver usato {}! Arrivederci.\n".format(header_title))
                return
            elif add_cof == 'back':
                ...
                while True:
                    try:
                        R = float(input("\nInserisci il raggio dell'albero (R) in metri (es. 0.05): ").strip())
                        if R <= 0:
                            print("Il raggio deve essere positivo. Riprova.")
                            continue
                        break
                    except ValueError:
                        print("Inserisci un numero valido (usa il punto come separatore decimale).")
                
                # 3.2) Carico radiale [N]
                while True:
                    try:
                        F = float(input("Inserisci il carico radiale (F) in Newton (es. 1000): ").strip())
                        if F <= 0:
                            print("Il carico deve essere positivo. Riprova.")
                            continue
                        break
                    except ValueError:
                        print("Inserisci un numero valido (usa il punto come separatore decimale).")
                
                # 3.4) Quale colonna corrisponde alla coppia (T, Nm) – X o Y?
                print("\nLe colonne attualmente disponibili nel file di output sono:")
                print("- X (valore medio o puntuale della colonna X selezionata)")
                print("- Y (valore medio o puntuale della colonna Y selezionata)")
                while True:
                    torque_col = input("Quale colonna corrisponde alla coppia (T, Nm)? (X/Y): ").strip().upper()
                    if torque_col == 'EXIT':
                        print("\nGrazie per aver usato {}! Arrivederci.\n".format(header_title))
                        return
                    elif torque_col == 'BACK':
                        break
                    if torque_col in ['X', 'Y']:
                        break
                    else:
                        print("Risposta non valida. Inserisci 'X' o 'Y'.")
                if torque_col == 'BACK':
                    break
                
                # 3.5) Quale colonna corrisponde alla velocità di rotazione (RPM) – X o Y?
                while True:
                    rpm_col = input("Quale colonna corrisponde alla velocità di rotazione (RPM)? (X/Y): ").strip().upper()
                    if rpm_col == 'EXIT':
                        print("\nGrazie per aver usato {}! Arrivederci.\n".format(header_title))
                        return
                    elif rpm_col == 'BACK':
                ...
                # 3.6) Calcolo delle nuove colonne
                # Assegna i valori dalle colonne X o Y
                if torque_col == 'X':
                    T = stats['X']
                else:
                    T = stats['Y']
                ...
                # Calcolo mm/s = R * (2*pi/60) * RPM * 1000
                # Nota: la velocità angolare ω = 2π * (RPM/60) = (2π/60)*RPM
                omega = (2 * math.pi / 60) * RPM
                mm_s = R * omega * 1000
                
                # Calcolo CoF = T / (F * R)
                # Attenzione: evitare divisione per zero
                with np.errstate(divide='ignore', invalid='ignore'):
                    CoF = T / (F * R)
                    # Sostituisci infiniti o NaN con 0 (opzionale, si può anche lasciare)
                    CoF = CoF.replace([np.inf, -np.inf], np.nan)
                
                # Aggiungi le colonne al dataframe stats
                stats['mm/s'] = mm_s
                stats['CoF'] = CoF
                
                print("\nColonne 'mm/s' e 'CoF' aggiunte con successo.")
                print("Anteprima dei nuovi valori (prime 10 righe):")
                print(stats[['mm/s', 'CoF']].head(10))
                break
                
            elif add_cof == 'n':
                print("Nessuna colonna aggiuntiva. Procedo al salvataggio.")
                break
            else:
                print("Risposta non valida. Rispondi 's', 'n', 'exit' o 'back'.")
        
        if add_cof == 'back':
            # Se l'utente ha premuto back, rimuoviamo questo step dallo stack e torniamo indietro
            if step_stack:
                step_stack.pop()
            continue
        
        # 12. Gestione righe vuote e salvataggio (ex passo 11)
        current_step = "12. Gestione righe vuote e salvataggio"
        print(f"\n=== {current_step} ===")
        step_stack.append(current_step)
        ...
        # 13. Scelta di uscita o continuazione
        print("\n" + "="*60)
